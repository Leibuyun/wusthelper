...
public class TokenFilter implements Filter {
     ...
     */
    @Value("#{'${wusthelper.verify.skip-authenticate-urls}'.split(',')}")
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        //获取token
        String token = "";
        if (uri.startsWith("/wustyjs")){
            token = request.getHeader("wustyjsToken");
        }else if (uri.startsWith("/wustlib")){
            token = request.getHeader("wustlibToken");
        }
